...
#include "i2c.h"
#include "hari.h"
//#include "mymacro.h" ( sudah ikut hari.h)
/******************************************************************************************/
void shiftout_jam(unsigned char a){
unsigned char b;
strobe_jam=0;
for(b=0;b<8;b++){
	if(a>=0x80)input_jam=1;
	else input_jam=0;
	a=(a<<1)|(a>>7);
	clock_jam=1;
	clock_jam=0;
...
strobe_tanggal=0;
for(b=0;b<8;b++){
	if(a>=0x80)input_tanggal=1;
	else input_tanggal=0;
	a=(a<<1)|(a>>7);
	clock_tanggal=1;
	clock_tanggal=0;
	}
...
void Baca_jadwal(void){
	unsigned int addr=0;
	switch (data[bulan]){
	   case 0x01 : addr = Jan;break;
	   case 0x02 : addr = Feb;break;
	   case 0x03 : addr = Mar;break;
	   case 0x04 : addr = Apr;break;
	   case 0x05 : addr = Mei;break;
	   case 0x06 : addr = Jun;break;
	   case 0x07 : addr = Jul;break;
	   case 0x08 : addr = Agu;break;
	   case 0x09 : addr = Sep;break;
	   case 0x10 : addr = Okt;break;
	   case 0x11 : addr = Nov;break;
	   case 0x12 : addr = Des;break;
	}
...
void load(){
	...
	if (segmen[3]==data_jam[0])segmen[3]=data_jam [10];//bulan
	if (segmen[9]==data_kalender[0])segmen[9]=data_kalender[10];//bulan
	if (segmen[11]==data_kalender[0])segmen[11]=data_kalender[10];//bulan
	...
	segmen[31]=data_kalender[10];//(data_jadwal[data[jamimsya]>>4]);
	...
	segmen[35]=data_kalender[10];//(data_jadwal[data[jamsubuh]>>4]);
...
if(data[jam]==data[jamimsya] && data[menit]==data[menitimsya]){
	...
	load();
	data[hari]=imsya;
	for(i=12;i<36;i++)if(i<28 || i>31)segmen[i]=data_jadwal[10];
	for(i=0;i<60;i++){
		if(i<10) buzer=1;
		else buzer=0;
		while(sqw==0){
...
//	port_flag=flag_subuh;
	...
	for(i=12;i<32;i++)segmen[i]=data_jadwal [10];
	goto alarm_on;
};
...
//	port_flag=flag_duhur;
	...
	for(i=12;i<36;i++)if(i<20 || i>23)segmen[i]=data_jadwal [10];
	goto alarm_on;
};
...
//	port_flag=flag_ashar;
	...
	for(i=12;i<36;i++)if(i<24 || i>27)segmen[i]=data_jadwal [10];
	goto alarm_on;
};
...
//	port_flag=flag_magrib;
	...
	for(i=16;i<36;i++)segmen[i]=data_jadwal [10];
	goto alarm_on;
};
...
//	port_flag=flag_isya;
	...
	for(i=12;i<36;i++)if(i<16 || i>19)segmen[i]=data_jadwal [10];
	goto alarm_on;
};
goto alarm_off;
alarm_on:
...
while(sqw==1);
while(mundur && tombol_iqomah==1){
	mundur--;
	if(bunyi){
		buzer=1;
		bunyi--;
	}
	while(sqw==0 && tombol_iqomah==1){
		tampil_hari(data[hari]);
		tampil();
	}
	tampil();
	while(sqw==1 && tombol_iqomah==1){
		buzer=0;
		tampil_hari(data[hari]);
	}
}
//COUNT DOWN IQOMAH
bacajam();
if(data[hari]==jumat)goto stanby;
buzer=1;
delay(500);
buzer=0;
data[hari]=iqomah;
...
//	port_flag=flag_iqomah;
	...
	for(j=59;j<60;j--){
		...
		load();
		for(i=12;i<36;i++)segmen[i]=data_jadwal [10];
		while(sqw==1);
		while(sqw==0){
			tampil_hari(data[hari]);
			tampil();
...
else mundur=60*readeprom(addr_stanby);

for(i=0;i<4;i++)segmen[i]=data_jam [10];
...
counter++;
if(counter==2){
	if(TH0<0x0a||TH0>0x0b)counter=0;
	data_ir=0;
}
if(counter>2 && counter<10){
	if(TH0>5 && TH0<8) data_ir++;
	if (TH0<4 || TH0>7 ) counter=0;
	data_ir=(data_ir<<1)|(data_ir>>7);
}
if (counter>9){
	counter=0;
	if (data_ir==0x08 || data_ir==0x48) up=0;
	if (data_ir==0x06) menu=0;
	if (data_ir==0x88 || data_ir==0xc8) down=0;
/*	//DEBUG INFO
	data[jam]=data_ir;
	data[menit]=data_ir;
	load();
...
while(1){
	...
	for(h=0;h<10;h++){
		...
		load();
		if(hidup<mati){
			if(!data[jam] && data[jam]<=hidup) power=0;
			if(data[jam]>=hidup && data[jam]<=mati) power=1;
			if(data[jam]>=mati && data[jam]<=0x23) power=0;
			}
		if(hidup>mati){
			if(!data[jam] && data[jam]<=mati) power=1;
			if(data[jam]>=mati && data[jam]<=hidup) power=0;
			if(data[jam]>=hidup && data[jam]<=0x23) power=1;};
		if(hidup==mati)power=1;
		if(power==0){
			segmen[0]=(data_jam[10]);
			segmen[1]=(data_jam[10]);
			segmen[2]=(data_jam[10]);
			segmen[3]=(data_jam[10]);
			for(h=4;h<12;h++) segmen[h]=data_jam[10];
			for(h=12;h<36;h++) segmen[h]=data_jadwal[10];
//			port_flag=flag_stanby;
			}
//		else port_flag=flag_normal;
		if(h<5)tampil_hari(data[hari]);
		while(sqw==1){
		//while(--i){
			tampil();
			tombol();
